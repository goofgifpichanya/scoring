...
import { Trophy, Medal, MessageSquare, ChevronDown, ChevronUp, Trash2, Eye, UserCheck } from 'lucide-react';
...
export const LeaderboardTable = ({ teamStats, onDeleteRecord }) => {
  ...
  const openTeamDetail = (stat) => {
    setSelectedTeamModal({
      cohortId: stat.cohortId,
  ...
  return (
    <>
          ...
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/90 text-slate-400 text-xs uppercase tracking-wider">
                ...
                <th className="py-4 px-4 font-semibold text-center">รายละเอียดคะแนน</th>
              </tr>
            </thead>
              ...
              {teamStats.map((stat) => {
                ...
                return (
                  <React.Fragment key={rowKey}>
                    <tr
                      onClick={() => openTeamDetail(stat)}
                      title="คลิกเพื่อดูคะแนนแยกรายกรรมการ"
                    ...
                    >
                      ...
                      <td className="py-4 px-4 font-bold text-white text-base">
                        <span className="px-2.5 py-1 rounded-lg bg-indigo-950/80 border border-indigo-500/40 text-indigo-200">
                          {stat.teamCode}
                        </span>
                      </td>
                      ...
                      <td className="py-4 px-4 text-center" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            onClick={() => openTeamDetail(stat)}
                            className="px-3 py-1.5 rounded-xl bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-200 border border-indigo-500/40 text-xs font-semibold transition-colors flex items-center gap-1"
                            title="ดูคะแนนแยกตามกรรมการแต่ละคน"
                          >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {stat.comments.map((c, i) => (
                                <div key={i} className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 space-y-1">
                                  <div className="text-xs font-semibold text-blue-400">🧑‍⚖️ {c.judgeName}</div>
      ...
      {selectedTeamModal && (
        <TeamDetailModal
          cohortId={selectedTeamModal.cohortId}
          cohortName={selectedTeamModal.cohortName}
          teamCode={selectedTeamModal.teamCode}
